...
const Post = ({ post: { id, ...post }, isPostPage, repliesCount }) => {
  ...
  return (
    <div className="p-3 cursor-pointer border-b dark:border-gray-600 pb-1">
      <Link href={`/${post.tag}/${id}`}>
        ...
        <div
          className={`${!isPostPage && 'hidden'} -mt-4 ml-11 text-[#71767b]`}
        >
          @{post.tag}
        </div>
        <div className={`${isPostPage ? 'ml-0' : 'ml-12'}`}>
          <div className={`${!isPostPage && '-mt-4'}`}>
            ...
            <time
              className={`${isPostPage ? 'block' : 'hidden'} text-[#71767b]`}
            >
